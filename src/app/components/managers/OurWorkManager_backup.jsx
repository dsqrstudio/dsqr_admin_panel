...
export default function OurWorkManager({ activeSub }) {
  ...
  const [selectedSubsection, setSelectedSubsection] = useState(initialSubsectionKey);
  ...
  const fetchMediaItems = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/admin/media-items/category/our_work`, {
        credentials: 'include'
      });
      ...
      if (response.ok) {
        ...
        if (result.success && Array.isArray(result.data)) {
          ...
          const loadedSections = JSON.parse(JSON.stringify(INITIAL_FEATURED_PROJECTS_STRUCTURE));
  ...
  function addItem() {
    ...
    if (!isValidUrl(newLink)) return pushToast("error", "Enter a valid http/https link.");
    ...
    setSections((s) => {
      ...
      const isVideo = newLink.includes("playlist.m3u8") || newLink.includes(".mp4");
  ...
  function doDelete() {
    ...
    setSections((s) => {
      const copy = JSON.parse(JSON.stringify(s));
      copy[sectionKey].categories[subName] = copy[sectionKey].categories[subName].filter((it) => it.id !== id);
      return copy;
    });
  ...
  function startEdit(item, sectionKey, subName) {
    setEditing({ 
      ...
      type: item.type || (item.src.includes('playlist.m3u8') ? 'video' : 'image'), 
      sectionKey, 
      subName, 
      src: item.src,
      poster: item.poster || '',
  ...
  function saveEdit() {
    ...
    if (!isValidUrl(editing.value)) return pushToast("error", "Enter a valid http/https link for SRC.");
    
    if (editing.type === 'video' && editing.posterValue && !isValidUrl(editing.posterValue)) {
  ...
  function addSubsection() {
    ...
    const defaultName = `Subsection ${Object.keys(currentCategories).length + 1}`;
    const name = newName || defaultName;
    
    if (!name) return pushToast("error", "Enter subsection name.");

    if (currentCategories[name]) {
        return pushToast("error", `Subsection "${name}" already exists in ${selectedSection}.`);
    }
  ...
  async function saveAll() {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/admin/media-items/bulk/${mediaCategory}`, {
        method: 'POST',
      ...
      if (response.ok) {
        pushToast("success", "Saved successfully!", `Updated ${itemsToSave.length} media items.`);
        await fetchMediaItems(); // Re-fetch to sync IDs and order
      } else {
        const errorData = await response.json();
        pushToast("error", "Save failed", errorData.error || "Server error occurred.");
  ...
  const featuredItems = sections[selectedSection]?.categories[selectedSubsection] || [];
  
  // NEW: Isolate Before/After items by their specific subsection name
  const beforeAfterItems = sections['Videos']?.categories['Before/After Video']?.map(item => ({
      id: item.id,
      before: item.before,
      after: item.after,
    ...
    <div className="max-w-7xl mx-auto p-4">
      ...
      <div className={active !== "Before/After Video" ? "block" : "hidden"}>
        <div className="mb-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <h2 className="text-2xl font-semibold">Our Work — Featured Projects</h2>

          <div className="flex items-center gap-2">
            <button
              onClick={() => setMode("preview")}
              className={`px-3 py-1 rounded font-medium border ${mode === "preview" ? "bg-black text-white" : "bg-white"}`}
            >
              Preview
            </button>
            <button
              onClick={() => setMode("add")}
              className={`px-3 py-1 rounded font-medium border ${mode === "add" ? "bg-black text-white" : "bg-white"}`}
            >
              Add Data
            </button>
        ...
        {!loading && (
          <>
            ...
            <div className="mb-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              ...
              <div>
                <label className="text-sm font-medium block mb-1">Subsection</label>
                <select
              ...
              <div>
                <label className="text-sm font-medium block mb-1">Add Subsection</label>
                <div className="flex gap-2">
                  ...
                  />
                  <button onClick={addSubsection} className="rounded px-3 py-2" style={{ background: PRIMARY }}>
                    + Add
                  </button>
                </div>
              </div>
              ...
              <div className="mb-6 rounded-lg border bg-white p-4">
                <div className="mb-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                  <div className="text-sm">
                    Add new item to <span className="font-semibold">{selectedSection} → {selectedSubsection}</span>
                  </div>
                  <div className="text-xs text-gray-500">Link must be a valid http/https URL.</div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  />
                  <div className="flex gap-2">
                    <button onClick={addItem} className="rounded px-4 py-2" style={{ background: PRIMARY }}>
                      Add
                    </button>
                    <button onClick={() => { setNewLink(""); pushToast("success", "Cleared"); }} className="rounded border px-4 py-2">
                      Clear
                    </button>
                  </div>
                </div>
            ...
            <div className="rounded-lg border bg-white p-4">
              <div className="mb-3 flex items-center justify-between">
                <div>
                  <h3 className="text-lg font-semibold">{selectedSection} — {selectedSubsection}</h3>
                  <div className="text-sm text-gray-500">{featuredItems.length} {featuredItems.length === 1 ? "item" : "items"}</div>
                </div>
                <div className="text-sm text-gray-500">Compact list view</div>
              </div>

              {featuredItems.length === 0 && <div className="text-sm text-gray-500">No items in this subsection yet.</div>}

              {featuredItems.length > 0 && (
                <div className="grid gap-3">
                  {featuredItems.map((it) => {
                    const previewSrc = (it.type === 'video' && it.poster) ? it.poster : it.src;

                    return (
                      <div key={it.id} className="flex items-center gap-3 p-3 rounded border-sm hover:shadow-sm transition-shadow">
                        <div className="flex-shrink-0 h-16 w-16 overflow-hidden rounded bg-gray-100">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={previewSrc} alt={it.alt || it.id} className="h-full w-full object-cover" />
                        </div>

                        <div className="flex-1 min-w-0">
                          {/* APPLY TRUNCATION FIX: truncate, overflow-hidden */}
                          <div className="text-sm font-medium truncate overflow-hidden" title={it.src}>{it.src}</div>
                          <div className="text-xs text-gray-500">
                            {it.type === 'video' ? `Video Source (${it.poster ? 'Poster Used' : 'No Poster'})` : 'Image Source'}
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2 items-center flex-shrink-0">
                          <button
                            onClick={() => { setMode("add"); setNewLink(it.src); pushToast("success", "Link copied to Add input"); }}
                            className="text-xs rounded px-3 py-1 border"
                            aria-label="Use in Add"
                          >
                            Use in Add
                          </button>
                          <button onClick={() => startEdit(it, selectedSection, selectedSubsection)} className="text-xs rounded px-3 py-1 border" aria-label="Edit">
                            Edit
                          </button>
                          <button onClick={() => requestDelete(selectedSection, selectedSubsection, it.id)} className="text-xs rounded px-3 py-1 border text-red-600" aria-label="Delete">
                            Delete
                          </button>
                        </div>
                      </div>
            ...
            {editing && (
              <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
                <div className="w-full max-w-2xl rounded-lg overflow-hidden shadow-lg bg-white">
                  <div className="flex items-center justify-between px-5 py-3" style={{ background: PRIMARY }}>
                    <div className="text-lg font-semibold">Edit item</div>
                    <button onClick={cancelEdit} className="rounded px-2 py-1 border" aria-label="Close">✕</button>
                  </div>
                  ...
                  <div className="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="md:col-span-1 flex items-center justify-center">
                      <div className="w-full max-w-xs">
                        <div className="h-40 w-full overflow-hidden rounded bg-gray-100">
                          ...
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={(editing.type === 'video' && editing.poster) ? editing.poster : editing.value} alt="preview" className="h-full w-full object-cover" />
                        </div>
                        <div className="mt-2 text-xs text-gray-500">Current thumbnail</div>
                      </div>
                    </div>
                    ...
                    <div className="md:col-span-2 flex flex-col gap-3">
                      ...
                      <label className="text-sm font-medium">
                        {editing.type === 'video' ? 'Video CDN Link (SRC)' : 'Image CDN Link (SRC)'}
                      </label>
                      <input
                        value={editing.value}
                      ...
                      {editing.type === 'video' && (
                        <>
                          <label className="text-sm font-medium mt-2">Poster Image Link (Optional Thumbnail)</label>
                          <input
                            value={editing.posterValue}
                            onChange={(e) => setEditing((p) => ({ ...p, posterValue: e.target.value }))}
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <button onClick={saveEdit} className="rounded px-4 py-2" style={{ background: PRIMARY }}>
                          Save changes
                        </button>
                        <button onClick={cancelEdit} className="rounded px-4 py-2 border">
                          Cancel
                        </button>
                        <button
                          onClick={() => { setConfirmDelete({ sectionKey: editing.sectionKey, subName: editing.subName, id: editing.id }); setEditing(null); }}
                          className="rounded px-4 py-2 border text-red-600"
                        >
                          Delete item
                        </button>
                      </div>
                      <div className="text-xs text-gray-500">Make sure the link is an accessible URL.</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
                <div className="w-full max-w-sm rounded bg-white p-5 shadow">
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={doDelete} className="rounded px-3 py-1" style={{ background: "#ff6b6b", color: "#fff" }}>Delete</button>
                  </div>
                </div>
              </div>
            )}
      ...
      <div className={active === "Before/After Video" ? "block mt-4" : "hidden"}>
        <div className="rounded-lg border bg-white p-6">
          <h2 className="text-2xl font-semibold mb-3">Before / After Videos</h2>
          <p className="text-sm text-slate-600 mb-4">
            Add pairs of thumbnails (Before / After). Each pair expects two CDN links (before + after).
          </p>
