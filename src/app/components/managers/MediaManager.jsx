...
export default function MediaManager({
  ...
  useEffect(() => {
    ...
    const timers = toastStack.map((t) => setTimeout(() => removeToast(t.id), 3000));
  ...
  function addItem() {
    if (beforeAfter) {
      if (!beforeInput || !afterInput || !beforePosterInput || !afterPosterInput) {
        pushToast("error", "All four links (Before/After Video and Posters) are required.");
        return;
      }
      if (!isValidUrl(beforeInput) || !isValidUrl(afterInput) || !isValidUrl(beforePosterInput) || !isValidUrl(afterPosterInput)) {
  ...
  function saveEditBA(id, before, after, beforePoster, afterPoster) {
    ...
    if (!isValidUrl(b) || !isValidUrl(a) || !isValidUrl(bp) || !isValidUrl(ap)) {
      pushToast("error", "Enter valid http/https links for all fields.");
      return;
    }
    if (old && old.before === b && old.after === a && old.beforePoster === bp && old.afterPoster === ap) {
      pushToast("info", "No changes detected");
      setEditing(null);
      return;
    }
    setItems((s) => s.map((it) => (it.id === id ? { ...it, before: b, after: a, beforePoster: bp, afterPoster: ap } : it)));
    setEditing(null);
  ...
  function Toast({ t }) {
    ...
    const bg = isError ? "bg-rose-50" : isSuccess ? "bg-emerald-50" : isDeleted ? "bg-rose-50" : "bg-slate-900";
    const border = isError ? "border-rose-200" : isSuccess ? "border-emerald-200" : isDeleted ? "border-rose-200" : "border-slate-900";
    const txt = isError ? "text-rose-700" : isSuccess ? "text-emerald-900" : isDeleted ? "text-rose-700" : "text-white";

    const Icon = isError ? FiTrash2 : isSuccess ? FiCheck : isDeleted ? FiTrash2 : FiInfo;

    return (
      <div className={`min-w-[220px] max-w-sm rounded-md px-4 py-2 shadow-md border ${bg} ${border}`}>
      ...
      <div className="rounded-lg overflow-hidden border bg-white shadow-sm">
        <div className="grid grid-cols-2 sm:grid-cols-2 gap-2 p-2 pb-0">
          <div className="relative aspect-square">
            ...
            <img src={srcBefore} alt="Before" className="object-cover w-full h-full" />
            <div className="absolute left-2 bottom-2 bg-black/60 text-white text-xs px-2 py-1 rounded">Before</div>
          </div>
          <div className="relative aspect-square">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={srcAfter} alt="After" className="object-cover w-full h-full" />
            <div className="absolute left-2 bottom-2 bg-black/60 text-white text-xs px-2 py-1 rounded">After</div>
          </div>
        </div>
        ...
        <div className="p-3 flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <div className="text-sm font-medium truncate" title={`Before: ${item.before} | After: ${item.after}`}>
    ...
    return (
      <div className="rounded-lg overflow-hidden border bg-white shadow-sm">
        <div className="relative aspect-square bg-gray-50">
          ...
          <div className="absolute left-2 bottom-2 bg-white/90 text-slate-800 text-xs px-2 py-1 rounded">Image</div>
        </div>

        <div className="p-3 flex items-center justify-between gap-3">
          <div className="min-w-0">
            <div className="text-sm font-medium truncate" title={it.src}>{truncate(it.src, 80)}</div>
            <div className="text-xs text-slate-400">Image URL</div>
          </div>

          <div className="flex items-center gap-2">
            <button onClick={() => startEdit(it)} className="rounded px-3 py-1 border text-xs bg-white hover:bg-slate-50" aria-label="Edit image">Edit</button>
            <button onClick={() => requestDelete(it.id)} className="rounded px-3 py-1 border text-xs text-rose-600 bg-white hover:bg-rose-50" aria-label="Delete image">Delete</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="rounded-lg bg-white p-4 shadow-sm max-w-7xl mx-auto">
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <div className="text-sm text-slate-500">Manage media — {beforeAfter ? "Before / After pairs" : "Single media items"}</div>
        </div>

        <div className="flex items-center gap-2">
          <button onClick={handleSaveAll} className="rounded bg-[#cff000] px-3 py-1 text-sm font-semibold">Save All</button>
        </div>
      </div>
      ...
      <div className="mb-6">
        {beforeAfter ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 items-end">
            ...
            <div>
              <label className="text-xs text-slate-600 block mb-1">Before Video link</label>
              <input
                value={beforeInput}
            ...
            <div>
              <label className="text-xs text-slate-600 block mb-1">After Video link</label>
              <input
                value={afterInput}
            ...
            <div>
              <label className="text-xs text-slate-600 block mb-1">Before Poster link</label>
              <input
            ...
            <div>
              <label className="text-xs text-slate-600 block mb-1">After Poster link</label>
              <input
            ...
            <div className="md:col-span-4 flex gap-2 justify-end">
              <button onClick={addItem} className="rounded bg-[#cff000] px-4 py-2 font-semibold">Add Pair</button>
              <button onClick={() => { setBeforeInput(""); setAfterInput(""); setBeforePosterInput(""); setAfterPosterInput(""); pushToast("success", "Cleared"); }} className="rounded border px-4 py-2">Clear</button>
            </div>
          </div>
        ) : (
          <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center">
            <div className="flex-1">
              <label className="text-xs text-slate-600 block mb-1">CDN link</label>
              <input
                value={input}
            ...
            <div className="flex gap-2">
              <button onClick={addItem} className="rounded bg-[#cff000] px-4 py-2 font-semibold">Add</button>
              <button onClick={() => { setInput(""); pushToast("success", "Cleared"); }} className="rounded border px-4 py-2">Clear</button>
            </div>
          </div>
        )}
      </div>
      ...
      {beforeAfter ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {items.length === 0 && <div className="text-sm text-slate-500">No pairs yet.</div>}
          {items.map((it) => (
        ...
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-2xl rounded-lg overflow-hidden bg-white shadow-lg">
            <div className="flex items-center justify-between px-5 py-3 bg-slate-900 text-white">
              ...
              <button onClick={() => setEditing(null)} className="rounded px-2 py-1 border border-white/10">✕</button>
            </div>
            ...
            <div className="p-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                <div className="text-center">
                  <div className="h-48 w-full overflow-hidden rounded bg-gray-100">
                    ...
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={editing.beforePoster || editing.before} alt="before" className="h-full w-full object-cover" />
                  </div>
                  <div className="mt-2 text-sm text-slate-700 font-semibold">Before Preview</div>
                </div>
                ...
                <div className="text-center">
                  <div className="h-48 w-full overflow-hidden rounded bg-gray-100">
                    ...
                    {/* eslint-disable-next-line @next/next/no-img-element */}
                    <img src={editing.afterPoster || editing.after} alt="after" className="h-full w-full object-cover" />
                  </div>
                  <div className="mt-2 text-sm text-slate-700 font-semibold">After Preview</div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* Before Video Link */}
                <div>
                  <label className="text-sm font-medium block mb-2">Before Video Link (SRC)</label>
                  <input 
                    value={editing.before} 
                    onChange={(e) => setEditing((p) => ({ ...p, before: e.target.value }))} 
                    className="w-full rounded border px-3 py-2" 
                ...
                {/* After Video Link */}
                <div>
                  <label className="text-sm font-medium block mb-2">After Video Link (SRC)</label>
                  <input 
                    value={editing.after} 
                    onChange={(e) => setEditing((p) => ({ ...p, after: e.target.value }))} 
                    className="w-full rounded border px-3 py-2" 
                    placeholder="e.g., video.m3u8"
                  />
                </div>
                ...
                {/* Before Poster Link */}
                <div>
                  <label className="text-sm font-medium block mb-2">Before Poster Link</label>
                  <input 
                    value={editing.beforePoster} 
                    onChange={(e) => setEditing((p) => ({ ...p, beforePoster: e.target.value }))} 
                    className="w-full rounded border px-3 py-2" 
                ...
                {/* After Poster Link */}
                <div>
                  <label className="text-sm font-medium block mb-2">After Poster Link</label>
                  <input 
                    value={editing.afterPoster} 
                    onChange={(e) => setEditing((p) => ({ ...p, afterPoster: e.target.value }))} 
                    className="w-full rounded border px-3 py-2" 
                    placeholder="e.g., thumbnail.jpg"
                  />
                </div>
                ...
                <div className="md:col-span-2 flex justify-end gap-2">
                  <button onClick={() => setEditing(null)} className="rounded px-4 py-2 border">Cancel</button>
                  <button onClick={() => saveEditBA(editing.id, editing.before, editing.after, editing.beforePoster, editing.afterPoster)} className="rounded px-4 py-2 bg-[#cff000]">Save</button>
                </div>
              </div>
            </div>
          </div>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-sm rounded bg-white p-5 shadow">
            ...
            <div className="text-sm text-gray-600 mb-4">Are you sure you want to permanently delete this item? This cannot be undone.</div>
            <div className="flex justify-end gap-2">
              <button onClick={cancelDelete} className="rounded px-3 py-1 border">Cancel</button>
              <button onClick={doDelete} className="rounded px-3 py-1 bg-rose-600 text-white">Delete</button>
            </div>
          </div>
        </div>
      )}
