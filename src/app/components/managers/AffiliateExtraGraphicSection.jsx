...
export default function AffiliateExtraGraphicSection({ subsection = 'affiliate', label = 'Affiliate — Extra Graphic' }) {
  ...
  const fetchImage = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/media-items/category/extras?subsection=${subsection}`, { credentials: 'include' });
  ...
  const handleReplace = async (file) => {
    ...
    form.append('file', file);
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/media-items/${image._id}/replace`, {
        method: 'POST',
        body: form,
        credentials: 'include',
      });
  ...
  const handleDelete = async () => {
    ...
    setReplacing(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/admin/media-items/${image._id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (res.ok) {
  ...
  return (
    <div className="flex flex-col items-center justify-center mt-8 w-full">
      <div className="rounded-2xl bg-white shadow-lg border p-8 max-w-lg w-full flex flex-col items-center">
        ...
        <p className="text-sm text-slate-600 mb-6 w-full text-center">Only one image is allowed. You can replace or delete it after upload.</p>
        {loading ? (
            ...
            <div className="relative w-full flex flex-col items-center">
              ...
              {(replacing || uploading) && (
                <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/70 rounded-xl">
                  ...
                  <span className="text-blue-700 font-medium">{replacing ? 'Processing...' : 'Uploading...'}</span>
                </div>
              )}
            </div>
            <div className="flex gap-3 mt-2">
              <label className="inline-flex items-center gap-1 px-4 py-2 rounded bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm font-medium cursor-pointer border border-blue-200 transition">
                ...
                <input
                  ...
                  onChange={(e) => {
                    if (e.target.files && e.target.files[0]) handleReplace(e.target.files[0]);
                    e.target.value = '';
                  }}
                />
              </label>
      ...
      <AlertDialog open={showDeleteModal} onOpenChange={setShowDeleteModal}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this affiliate extra graphic? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <div className="mt-4 flex justify-end gap-2">
            <AlertDialogCancel asChild>
              <button className="px-4 py-2 rounded border bg-white text-slate-700 font-medium">Cancel</button>
            </AlertDialogCancel>
            <AlertDialogAction asChild>
              <button
                ...
                disabled={replacing}
              >
                {replacing ? <FiLoader className="animate-spin inline mr-2" /> : null} Delete
              </button>
            </AlertDialogAction>
          </div>
        </AlertDialogContent>
